...
var absolutePath =  path.relative('./','/home/vagrant/.ethereum/geth.ipc');
var TransactionService = require('./../services/transaction.service');
var Transaction = require('./../models/transaction.model');
var decimal = 10000000000;
var options = {
  // host : absolutePath,
...
var contractAddress= "0xde7ba6d4e76b5a21a84edded794d6e13f721e585";

exports.checkEthBalance = function(req, res) {

	var accountAddr = req.body.accountAddress;
	if(isAddress(accountAddr)==false){
		console.log("This address is not valid",accountAddr);
		return res.json({"success":false,"data":[{"message":"Invalid address"}]});

	}
	var ethBalance=web3.fromWei(web3.eth.getBalance(accountAddr),"ether");
...
exports.checkCoinBalance = function(req, res) {
    ...
    coinBalance = coinBalance / decimal;
	...
	web3.personal.unlockAccount(from,passphrase,
	    function(_error,_resp){
			if(_error){
				...
				var coinBalance = contract.balanceOf(from).toNumber();
                coinBalance = coinBalance / decimal;
				...
				if(estimatedEther && ethBalance < estimatedEther){
                  ...
                  console.log("Coin balance is too low.", coinBalance, amount);
...
exports.createNewAccount = function (req, res){
	...
	var accountAddress = web3.personal.newAccount(passphrase, function(_error,_r){

		if(_error){
			return res.json({"success":false,"data":[{"message":"Error in creating account."}]});
		}
		else{
                return res.json({"success":true,"data":[{accountAddress:_r}]});
		}
	});
};

// exports.getPrivateKey = function (req, res){
//     var passphrase = req.body.passphrase;
//     var address = req.body.address;
//     var keyObject = keythereum.importFromFile(address, datadir, function (keyObject) {
...
//     //
//     //     console.log(_r);
//     //     if(privateKey.toString('hex').length){
//     //         return res.json({"success":true,"data":[{"privateKey":privateKey.toString('hex')}]});
//     //     }
//     //     else{
...
function checkThrow(frm,callData,gasPrice){
	...
	// var estimatedGas=web3.eth.estimateGas({to:contractAddress,data:callData});
